...
class PosOrder(models.Model):
    ...
    def _prepare_picking_vals(self, order):
        """
        ...
        if picking_type:
            ...
            if pos_qty:
                picking_vals = vals.copy()
                # order_picking = Picking.create(picking_vals.copy())
                # order_picking.message_post(body=message)
            neg_qty = any([x.qty < 0 for x in order.lines if x.product_id.type in ['product', 'consu']])
            if neg_qty:
                return_vals = vals.copy()
                return_vals.update({
                    'location_id': destination_id,
                    'location_dest_id': return_pick_type != picking_type and return_pick_type.default_location_dest_id.id or location_id,
                    'picking_type_id': return_pick_type.id
                })
            return picking_vals, return_vals
        else:
            return False, False

    def _prepare_move_vals(self, order, line, order_picking, return_picking):
        """
        ...
        vals = {
            ...
            'location_dest_id': destination_id if line.qty >= 0 else \
                return_pick_type != picking_type and return_pick_type.default_location_dest_id.id or location_id,
        }
        return vals
    ...
    def create_picking(self):
        ...
        for order in self:
            ...
            picking_vals, return_vals = self._prepare_picking_vals(order)
            if picking_vals:
            ...
            for line in order.lines.filtered(lambda l: l.product_id.type in ['product', 'consu'] and not float_is_zero(l.qty, precision_rounding=l.product_id.uom_id.rounding)):
                moves |= Move.create(self._prepare_move_vals(order, line, order_picking, return_picking))
